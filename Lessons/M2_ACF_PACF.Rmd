...
inflow_ts_plot <- ggplot(inflow_data, aes(x=my_date, y = HP1)) +
  geom_line(color = "blue")

#adding two time series to the same plot
ggplot(inflow_data, aes(x=my_date, y = HP1)) +
  geom_line(aes(y=HP1), color = "blue") + 
  geom_line(aes(y=HP2), color = "navy")
...
```{r}
ts_inflow_data <- ts(inflow_data[,"HP1"], start = c(1931, 1), frequency = 12)
ts_inflow_data
...
HP1_acf <- Acf(ts_inflow_data)

HP1_pacf <- Pacf(ts_inflow_data)
  
```

```{r}
#ACF & PACF calculation practice

Temp <- c(14.2,16.4,11.9,15.2,18.5,22.1,19.4,25.1,23.4,18.1,22.6,17.2)
ts_Temp <- ts(Temp)
Acf_Temp <- Acf(ts_Temp)
Acf_Temp
```
